...
and calculates LPIPS (Learned Perceptual Image Patch Similarity) scores using VGG backbone.
...
def evaluate_lpips_scores(
    source_folder: str,
    output_csv: str,
) -> None:
    """
    Evaluate LPIPS scores between no_intervention and intervention images.
    ...
    # Initialize LPIPS model with VGG backbone (default learned weights)
    print("Loading LPIPS model with VGG backbone...")
    lpips_model = lpips.LPIPS(net="vgg").to(device)
    lpips_model.eval()
    print("✓ LPIPS model loaded successfully")

    # Image transformation - LPIPS expects images in range [-1, 1]
    ...
    try:
        ...
        for i, (baseline_path, intervention_path, params) in enumerate(comparisons, 1):
            try:
                ...
                baseline_tensor = transform(baseline_img).unsqueeze(0).to(device)
                intervention_tensor = transform(intervention_img).unsqueeze(0).to(device)

                # Calculate LPIPS score
                with torch.no_grad():
                    lpips_score = lpips_model(baseline_tensor, intervention_tensor).item()
    ...
    if num_processed > 0:
        print(f"Average LPIPS score: {total_score / num_processed:.4f}")
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate LPIPS scores between baseline and intervention images"
    )
    parser.add_argument(
        "--source_folder",
        type=str,
